## Gene lists (from selection tests) --> orthology --> GO Terms --> Enrichment
...
### Create inputput gene lists (BUSCO genes IDS) (this example)
```
cd /scratch/ek/comparative/2023phylogenyB
grep "Evidence for \*relaxation of selection" ./07.hyphy.relax/*.stdout.log | wc -l
grep "Evidence for \*relaxation of selection" ./07.hyphy.relax/*.stdout.log | cut -d":" -f1 | cut -d"/" -f3 | cut -d"." -f1 > ./07.hyphy.relax.completed.genenames.relaxation.txt
grep "Evidence for \*intensification of selection" ./07.hyphy.relax/*.stdout.log | cut -d":" -f1 | cut -d"/" -f3 | cut -d"." -f1 > ./07.hyphy.relax.completed.genenames.intensification.txt
grep "No significant evidence for relaxation (or intensification)" ./07.hyphy.relax/*.stdout.log | cut -d":" -f1 | cut -d"/" -f3 | cut -d"." -f1 > ./07.hyphy.relax.completed.genenames.nochange.txt
```
...
### create GO Term lists via BioMaRT
```
cd /scratch/ek/comparative/2023phylogenyB
cd orthologs.apis.broccoli
QUERYSPECIES="BombusTerrestris"
...
## run TopGo enrichment analysis (change node size to gauge strictness)
...
### visualize results with [GOfigure](https://gitlab.com/evogenlab/GO-Figure)
```
######
gofigure.py --help
