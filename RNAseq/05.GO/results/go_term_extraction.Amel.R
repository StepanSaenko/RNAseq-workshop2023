...
#- Column 2 - ensembl reference eg Apis (RefSpecies) protein ID for predicted homolog
...
inputfile = "/scratch/rnaseq2023/estolle/go/Apis.mellifera.transcriptIDsfromGTF2.txt"    #replace with path to file (eg on commandline via sed)
...
ensembl <- useEnsemblGenomes(biomart = "metazoa_mart", dataset = "amellifera_eg_gene")
...
## Write to file:
write.table(query_ref_go_terms, file = paste(inputfile,'.GO.Amel.raw.txt', sep = ''), quote = FALSE, sep = "\t", row.names = FALSE, col.names = FALSE)
...
dros_ref_homologs <- getBM(attributes = c('ensembl_gene_id', 'amellifera_eg_homolog_ensembl_gene', 'amellifera_eg_homolog_ensembl_peptide'), mart = dros_ensembl)
...
dros_go_terms$amellifera_eg_homolog_ensembl_peptide <- dros_ref_homologs[match(dros_go_terms$ensembl_gene_id, dros_ref_homologs$ensembl_gene_id), ]$amellifera_eg_homolog_ensembl_peptide
...
dros_go_terms$query_prot <- query_data[match(dros_go_terms$amellifera_eg_homolog_ensembl_peptide, query_data$ref_prot), ]$query_prot
...
## Write to file:
write.table(query_dros_go_terms, file = paste(inputfile,'.GO.Amel-Dmel.raw.txt', sep = ''), quote = FALSE, sep = "\t", row.names = FALSE, col.names = FALSE)
